...
for record in csv_list:
    ...
    if (record[2] == '2'):
        if (record[3] == '2'):
            for x in range(4, len(record)):
                input_vector.append(float(record[x]))
            output_vector = float(record[0])
...
file = open("dota2_nonboost_max_depth_results.csv", "w")
ts = time.time()
...
file = open("dota2_adaboost_n_estimators_results.csv", "w")
...
file = open("dota2_adaboost_max_depth_results.csv", "w")
...
file = open("dota2_k_results.csv", "w")
